...
app.post('/signin', function(req, res) {
  ...
  var checker = "SELECT id, username, password, is_admin FROM users WHERE username = '" + reqUsername + "'";
  db.all("SELECT id, username, password, is_admin FROM users WHERE username = '" + reqUsername + "'", function(err, rows) {
    if(err) {
      console.log("checker1 is: " + checker);
      throw err;
    }
    if(!rows || rows.length > 1) {
      console.log("checker2 is: " + checker);
      throw "this shouldn't happenzz";
    }

    console.log("checker3 is: " + checker);
